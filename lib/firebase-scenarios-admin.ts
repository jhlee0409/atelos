// Firestore 시나리오 서비스 - Admin SDK (서버 전용)
// 보안 규칙을 우회하여 모든 시나리오에 접근 가능
...
function timestampToISOString(timestamp: FirebaseFirestore.Timestamp | undefined): string | undefined {
...
export async function getAllScenarios(): Promise<ScenarioSummary[]> {
  try {
    ...
    console.error('❌ [Firebase Admin] 시나리오 목록 조회 실패:', error);
    throw error;
  }
}
...
export async function getScenario(scenarioId: string): Promise<ScenarioData | null> {
  try {
    ...
    console.error(`❌ [Firebase Admin] 시나리오 조회 실패 (${scenarioId}):`, error);
    throw error;
  }
}
...
export async function createScenario(scenario: ScenarioData): Promise<string> {
  try {
    ...
    console.log(`✅ [Firebase Admin] 시나리오 생성: ${scenario.scenarioId}`);
    return scenario.scenarioId;
  } catch (error) {
    console.error('❌ [Firebase Admin] 시나리오 생성 실패:', error);
    throw error;
  }
}
...
export async function updateScenario(scenario: ScenarioData): Promise<string> {
  try {
    ...
    console.log(`✅ [Firebase Admin] 시나리오 업데이트: ${scenario.scenarioId}`);
    return scenario.scenarioId;
  } catch (error) {
    console.error('❌ [Firebase Admin] 시나리오 업데이트 실패:', error);
    throw error;
  }
}
...
export async function deleteScenario(scenarioId: string): Promise<void> {
  try {
    ...
    const docRef = db.collection(SCENARIOS_COLLECTION).doc(scenarioId);
    await docRef.delete();
    console.log(`✅ [Firebase Admin] 시나리오 삭제: ${scenarioId}`);
  } catch (error) {
    console.error(`❌ [Firebase Admin] 시나리오 삭제 실패 (${scenarioId}):`, error);
    throw error;
  }
}
...
export async function scenarioExists(scenarioId: string): Promise<boolean> {
  try {
    ...
    console.error(`❌ [Firebase Admin] 시나리오 존재 확인 실패 (${scenarioId}):`, error);
    throw error;
  }
}
